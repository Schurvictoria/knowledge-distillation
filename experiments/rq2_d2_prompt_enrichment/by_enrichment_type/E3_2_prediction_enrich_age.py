#!/usr/bin/env python3
"""
RQ2 D2 Prediction enrichment Age (4-class). Qwen2.5-7B via OpenRouter.
XGBoost predicts class 0/1/2/3 with confidence → injected into prompt.
"""
...
class Budget:
    ...
    def add(self, pt, ct):
        ...
        self.calls += 1
        if self.calls % 50 == 0:
            print(f"    [BUDGET] ${self.spent:.3f}/${self.cap:.2f} ({self.calls})", flush=True)
    def check(self):
        if self.spent > self.cap:
            print(f"    [BUDGET EXCEEDED]", flush=True); return False
        return True
...
def run(api_key, n=500):
    print(f"=== Prediction-CoT Age (Qwen2.5-7B via OpenRouter) ===", flush=True)
