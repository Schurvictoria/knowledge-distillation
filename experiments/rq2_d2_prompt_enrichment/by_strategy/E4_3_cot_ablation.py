#!/usr/bin/env python3
"""
CoT Reasoning Effect: thinking on/off ablation for Qwen3.6 and GLM-4.7.

We already have:
- Qwen3.6 + kNN (default behavior, no explicit thinking toggle): 0.711
- GLM-4.7 + kNN (reasoning=False): 0.547

Need:
- Qwen3.6 with thinking=True vs False
- GLM-4.7 with thinking=True vs False (False is cached as 0.547)

Uses same prompts as run_openrouter_experiments.py.
"""
...
def call_with_thinking(model_id, messages, api_key, pos_label, neg_label, thinking_mode):
    ...
    for attempt in range(3):
        try:
            ...
            # Parse reasoning if content empty (for thinking=on)
...
def run_experiment(model_key, thinking_mode, dataset_name, api_key):
    """Run one experiment: model × thinking_mode × kNN enrichment."""
    m = MODELS[model_key]
    ...
    if cache_file.exists():
        ...
        print(f"  {m['name']} thinking={thinking_mode}: AUC={cached['auc']:.4f} (cached)", flush=True)
        return cached["auc"]

    print(f"\n=== {m['name']} thinking={thinking_mode} + kNN ===", flush=True)

    # Build system & messages (match main script)
    ...
    print(f"  {m['name']} thinking={thinking_mode}: AUC={auc:.4f}", flush=True)
...
if __name__ == "__main__":
    ...
    print(f"CoT Reasoning Effect: {args.dataset}, budget ${args.budget}", flush=True)

    results = {}
    # Qwen3.6 supports both modes
