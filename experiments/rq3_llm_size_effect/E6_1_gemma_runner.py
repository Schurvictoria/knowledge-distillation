#!/usr/bin/env python3
"""RQ3 D2: Gemma 2B with kNN enrichment on Gender. For size effect table."""
...
def run(dataset_name, strategy, api_key):
    print(f"\n=== Gemma 2B {strategy} on {dataset_name} ===", flush=True)
    ...
    if cache.exists():
        ...
        print(f"  Cached: AUC={c['auc']:.4f}", flush=True); return
    ...
    if ckpt.exists():
        ...
        start = len(preds); print(f"  Resuming from {start}", flush=True)
    else:
        preds = []; start = 0
    ...
    print(f"  Gemma 2B {strategy} {dataset_name}: AUC={auc:.4f}", flush=True)
