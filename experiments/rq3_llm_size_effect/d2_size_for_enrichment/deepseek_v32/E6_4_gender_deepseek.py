#!/usr/bin/env python3
"""
DeepSeek-V3.2-Speciale with proper max_tokens=8192 (AWS/HF recommendation).

Strategies: zero_shot_knn, few_shot_random, few_shot_knn, few_shot_cot_knn
...
def call_deepseek(messages, api_key, pos_label, neg_label, cot=False, seed=42):
    """Call DeepSeek-V3.2-Speciale with proper max_tokens and JSON schema."""
    ...
    for attempt in range(3):
        try:
            ...
            budget.add(usage.get("prompt_tokens", 500),
                      usage.get("completion_tokens", 100), "deepseek/deepseek-v3.2-speciale")
            if not budget.check(): return 0.5

            # Parse JSON from content (DeepSeek puts final answer there)
            try:
                ...
                # Fallback: search in content + reasoning for JSON
...
def run_strategy(strategy, dataset_name, api_key):
    print(f"\n=== DeepSeek-V3.2 {strategy} on {dataset_name} ===", flush=True)
    ...
    cache = OUT / f"{dataset_name}_deepseek_v3_{strategy}.json"
    if cache.exists():
    ...
    def process_one(i):
        ...
        # Short system (DeepSeek prefers everything in user)
        ...
        return i, call_deepseek(messages, api_key, data["pos_label"], data["neg_label"], cot=is_cot, seed=42)

    ckpt = OUT / f"{dataset_name}_deepseek_v3_{strategy}_ckpt.npz"
    if ckpt.exists():
    ...
    # Sanity check only on fresh run
    if start == 0:
        print("  Sanity check...", flush=True)
        for i in range(3):
            _, prob = process_one(i)
            true = data["pos_label"] if data["y_test"][i] == 1 else data["neg_label"]
            print(f"    Client {i}: true={true}, prob={prob:.3f}", flush=True)

    BATCH = 3  # Lower for DeepSeek (slower)
    t0 = time.time()
    ...
    np.savez(OUT / f"{dataset_name}_deepseek_v3_{strategy}_preds.npz",
...
if __name__ == "__main__":
    ...
    ap.add_argument("--strategies", nargs="*", default=None,
                    help="Override strategy list (default: all 4)")
    args = ap.parse_args()
    ...
    strats = args.strategies or ["zero_shot_knn", "few_shot_random", "few_shot_knn", "few_shot_cot_knn"]
    ...
    print("\n" + "="*60)
    print("FINAL SUMMARY (DeepSeek-V3.2-Speciale, max_tokens=8192)")
    print("="*60)
