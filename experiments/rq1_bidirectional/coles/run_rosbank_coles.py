...
from distil.data import load_rosbank_dataset, save_coles_embeddings
...
def main() -> None:
    ...
    print(f"  {coles_config.rnn_type.upper()}-{coles_config.hidden_size}, "
          ...
          f"slices=[{coles_config.minimum_sequence_length},{coles_config.maximum_sequence_length}], "
          f"noise={coles_config.embeddings_noise}")
    print(f"  LR scheduler: step={coles_config.lr_step_size}, gamma={coles_config.lr_gamma}")
    print(f"  amount: log-transformed, emb dims: {coles_config.embedding_dimensions}")
    print(f"  Seeds: {SEEDS}")
    ...
    for seed in SEEDS:
        ...
        dataset = load_rosbank_dataset(seed=seed)
        ...
        print(f"  training CoLES: {coles_config.num_epochs} epochs, "
              f"{coles_config.rnn_type.upper()}-{coles_config.hidden_size}, "
        ...
        for classifier_name, metric_dict in downstream_metrics.items():
            ...
            print(f"  {classifier_name:<8} AUC={metric_dict['roc_auc']:.4f}")
    ...
    metrics_dataframe.to_csv(OUTPUT_DIRECTORY / "rosbank_coles_per_seed.csv", index=False)

    aggregated_rows = []
    for (model_name, variant), grouped_subset in metrics_dataframe.groupby(["model", "variant"]):
        row = {"model": model_name, "variant": variant, "n_seeds": len(grouped_subset)}
        for metric_name in ["roc_auc", "accuracy", "f1"]:
            row[f"{metric_name}_mean"] = grouped_subset[metric_name].mean()
            row[f"{metric_name}_std"] = grouped_subset[metric_name].std()
