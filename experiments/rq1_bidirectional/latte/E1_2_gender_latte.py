"""E1.2 — LATTE on Gender (contrastive alignment with frozen LLM4ES embeddings)."""
import gc
import json
import time
import warnings
...
def _load_aligned_llm4es_embeddings(
...
def main() -> None:
    ...
    train_customer_ids_full = [record["customer_id"] for record in full_dataset.train_records]
    ...
    print(f"  train={len(train_records)}, val={len(val_records)}, test={len(full_dataset.test_records)}")
    ...
    print("\nPhase 1: Train CoLES baseline" if not COLES_CHECKPOINT_PATH.exists() else "\nPhase 1: Loading CoLES checkpoint")
    ...
    print(f"  Baseline CoLES LGBM AUC: {baseline_test_score:.4f}")

    # alpha=0.1 is canonical for E1.2 — matches REPORT.md=0.8674 for Gender
    ...
    print(f"\nBest: val={finetune_results['best_val_score']:.4f} "
          f"test={best_test_score:.4f} at epoch {finetune_results['best_epoch']}")

    summary_payload = {
        "baseline_coles": baseline_test_score,
        f"finetune_alpha{CANONICAL_DISTILLATION_WEIGHT}": best_test_score,
    }
    print("\nTRUE LATTE DISTILLATION SUMMARY")
    for method_name, method_score in sorted(summary_payload.items(), key=lambda pair: -pair[1]):
        delta = method_score - baseline_test_score
        delta_sign = "+" if delta >= 0 else ""
        print(f"  {method_name:<25} AUC={method_score:.4f} ({delta_sign}{delta:.4f})")

    with (OUTPUT_DIRECTORY / "true_latte_results.json").open("w") as file_handle:
        json.dump(summary_payload, file_handle, indent=2)
    pd.DataFrame(
        [{"method": name, "auc": value} for name, value in summary_payload.items()]
