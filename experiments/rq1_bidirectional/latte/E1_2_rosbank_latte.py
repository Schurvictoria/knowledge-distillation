"""E1.2 — LATTE on Rosbank (contrastive alignment with frozen LLM4ES embeddings)."""
import gc
import json
import time
import warnings
...
def _compute_baseline_lgbm_score(encoder, train_records, test_records, train_targets, test_targets, device):
    ...
    return float(roc_auc_score(test_targets, classifier.predict_proba(scaled_test)[:, 1]))
...
def main() -> None:
    ...
    # alpha=0.1 is canonical for E1.2 — matches REPORT.md=0.8057 for Rosbank
