#!/usr/bin/env python3
"""
Age LLM4ES v2: stronger fine-tuning (5 epochs, full 27k train data).
Previous: 2 epochs, 8k sample → standalone acc 0.409.
Goal: better LLM embeddings → bidirectional works.

Pipeline: fine-tune → extract embeddings → bidirectional distillation.
Saves checkpoints at each stage + pushes to git.
"""
...
OUTPUT_DIR = Path("results/age_llm4es_v2")
...
# ---- STEP 2: Fine-tune LLM (5 epochs, ALL train data) ----
print("\n" + "=" * 60)
print("STEP 2: Fine-tune Qwen2.5-3B (5 epochs, 27k clients)")
print("=" * 60)

MODEL_ID = "Qwen/Qwen2.5-3B"
LORA_CKPT = CKPT_DIR / "llm4es_v2_lora"
...
if (LORA_CKPT / "adapter_model.safetensors").exists():
    ...
    # ALL train clients (not 8k sample like v1)
...
EMB_PATH = OUTPUT_DIR / "llm4es_v2_embeddings.npz"
if EMB_PATH.exists():
...
results["llm4es_v2_only"] = accuracy_score(y_test, lgbm.predict(sc_l.transform(llm_test)))

# v1 LLM only (for comparison)
v1 = np.load("results/age_llm4es/llm4es_embeddings.npz")["embeddings"].astype(np.float32)
v1_tr, v1_te = v1[:n_tr], v1[n_tr:]
lgbm = LGBMClassifier(**LGBM_P, random_state=42)
lgbm.fit(sc_l.fit_transform(v1_tr), y_train)
results["llm4es_v1_only"] = accuracy_score(y_test, lgbm.predict(sc_l.transform(v1_te)))
...
results["concat_v2"] = accuracy_score(y_test, lgbm.predict(Xte))

print(f"  CoLES only:     {results['coles_only']:.4f}")
print(f"  LLM4ES v1 only: {results['llm4es_v1_only']:.4f}")
print(f"  LLM4ES v2 only: {results['llm4es_v2_only']:.4f}")
print(f"  Concat v2:      {results['concat_v2']:.4f}")

# ---- STEP 5: Bidirectional with v2 embeddings ----
print("\n" + "=" * 60)
