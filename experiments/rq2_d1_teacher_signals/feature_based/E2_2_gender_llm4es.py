#!/usr/bin/env python3
"""
Phase 3d: LLM4ES-style fine-tuning on Gender transactions.
Fine-tune LLaMA-3.2-3B on serialized transaction texts (next-token prediction).
Extract embeddings (mean pool last 8 layers) → downstream LGBM.
Then contrastive alignment with CoLES.
...
# Serialize transactions as text (LLM4ES-style: with MCC descriptions)
def serialize_client_llm4es(cid, max_txns=50):
    """Serialize client transactions with MCC descriptions."""
    if cid not in grouped.groups:
        return "No transactions."
    ct = grouped.get_group(cid)
    if len(ct) > max_txns:
        ct = ct.tail(max_txns)  # last N transactions
...
if not SKIP_FINETUNE:
    ...
    # Prepare dataset: all client texts (train only, to avoid data leakage)
...
# ---- Extract embeddings (LLM4ES-style: mean pool last 8 layers) ----
print("\n" + "=" * 60)
...
if EMB_PATH.exists():
    ...
    hidden_size = model.config.hidden_size  # 3072 for LLaMA-3.2-3B
    n_layers_pool = 8
    ...
    for i, cid in enumerate(all_cids):
        ...
        with torch.no_grad():
            ...
            # Mean pool last 8 layers
            hidden_states = outputs.hidden_states[-n_layers_pool:]  # tuple of (1, seq_len, hidden)
            stacked = torch.stack(hidden_states).mean(0)  # (1, seq_len, hidden)
...
# Contrastive adapter (LATTE-style with LLM4ES embeddings)
