#!/usr/bin/env python3
"""
OpenRouter experiments for RQ3 + RQ1.

Runs multiple LLMs via OpenRouter API on:
1. RQ3 Dir2: Zero-shot and Zero-shot + kNN CoT on Gender/Rosbank/Age
2. RQ3 CoT Effect: Thinking on/off for DeepSeek-R1 and Qwen3.6
3. RQ1: RAMD with strong teacher (get OOF predictions for later KD)

Usage:
  export OPENROUTER_API_KEY=sk-or-...
  python run_openrouter_experiments.py --experiment rq3_dir2 --dataset gender
  python run_openrouter_experiments.py --experiment rq3_cot --dataset gender
  python run_openrouter_experiments.py --experiment rq1_ramd_oof --dataset gender
"""
...
# ---- Budget tracking ----
class BudgetTracker:
    """Track API spending and stop if budget exceeded."""
    ...
    def add(self, input_tokens, output_tokens, model_id):
        ...
        pricing = {
            ...
            "google/gemma-4-26b-a4b-it:free": (0.0, 0.0),  # Free!
        ...
        if self.calls % 100 == 0:
            print(f"    [BUDGET] ${self.total_spent:.3f} / ${self.max_budget:.2f} "
                  f"({self.calls} calls)", flush=True)
        return self.total_spent <= self.max_budget

    def check(self):
        if self.total_spent > self.max_budget:
            print(f"\n    [BUDGET EXCEEDED] ${self.total_spent:.3f} > ${self.max_budget:.2f}. Stopping.", flush=True)
            return False
        return True

    def summary(self):
        print(f"\n    [BUDGET SUMMARY] Total: ${self.total_spent:.3f}, Calls: {self.calls}", flush=True)

budget = BudgetTracker(max_budget_usd=5.0)  # DEFAULT $5 limit
...
def call_openrouter_logits(model_id, messages, api_key, pos_label, neg_label, thinking=None, seed=42):
    """Call OpenRouter and parse response to get P(positive).

    Uses JSON-format response for reliable parsing.
    Returns probability for positive class.
    """
    headers = {
    ...
    payload = {
        "model": model_id,
        "messages": messages,
        # Reasoning models (GLM, DeepSeek): content=None with low max_tokens,
        # need enough for reasoning + "ANSWER: male" output
        # GLM supports disabling reasoning; DeepSeek-Speciale requires reasoning
        "max_tokens": 500 if thinking else (300 if "deepseek" in model_id else 10),
        "temperature": 0,
        "seed": seed,
        **({"logprobs": True, "top_logprobs": 20} if not thinking else {}),
    }
    # Disable reasoning for GLM (supports it); DeepSeek-Speciale requires reasoning
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code != 200:
                ...
                if attempt == 2:
                    return None  # Sentinel: hard failure, caller must skip
                time.sleep(2)
                continue
            data = resp.json()
            ...
            # Some models (GLM, DeepSeek) put everything in reasoning, content=None
            if not content.strip() and msg.get("reasoning"):
                reasoning = msg["reasoning"]
                r_lower = reasoning.lower()
                pos_l = pos_label.lower()
                neg_l = neg_label.lower()
                # Try 1: look for "ANSWER:" marker
                if "answer:" in r_lower:
                    ...
                    if pos_l in after_answer[:20]:
                        ...
                        content = neg_label
                # Try 2: look at last 50 chars for final answer
                if not content.strip():
                    ...
                    if last_pos > last_neg:
                        ...
                        content = neg_label
                # Try 3: full reasoning last occurrence
                if not content.strip():
            ...
            if not budget.check():
                return None  # Sentinel: budget exhausted, don't mark as done

            # Method 1: Try logprobs (best — continuous probability)
            ...
            # Method 2: Fallback — parse text response
            ...
            if attempt == 2:
                ...
                return None  # Sentinel: hard failure
            time.sleep(2 * (attempt + 1))
    return None


def load_dataset(dataset_name):
    """Load embeddings, labels, and transaction data."""
...
def run_rq3_dir2(dataset_name, model_keys=None, api_key=None, full_matrix=False):
    ...
    full_matrix=False: only zero-shot ± kNN (cheap, 2 prогона per LLM)
    full_matrix=True:  3 strategies × 4 enrichments (12 прогонов per LLM)
    """
    print(f"\n{'='*60}")
    ...
    # Build SHAP context (needed for shap/both enrichment)
    shap_ctx = None
    if full_matrix:
        from xgboost import XGBClassifier
        ...
        print("  SHAP context built.", flush=True)

    # Few-shot examples (fixed from train)
    few_shot_text = None
    if full_matrix:
        y_train = np.load(f"embeddings/{dataset_name}/y_train_seed42.npy")
        cids_train = np.load(f"embeddings/{dataset_name}/cids_train_seed42.npy")
    ...
    # Define strategies and enrichments
    if full_matrix:
        ...
        for strategy in strategies:
            for enrichment in enrichments:
                ...
                if cache_file.exists():
                    ...
                    print(f"  {strategy}×{enrichment}: AUC={cached['auc']:.4f} (cached)", flush=True)
                    continue

                # System prompt: short, domain context only
                ...
                if checkpoint_file.exists():
                    ...
                    start_idx = len(preds)
                    print(f"  Resuming {strategy}×{enrichment}: {start_idx}/{len(data['cids_test'])}", flush=True)
                else:
                    preds = []
                    start_idx = 0
                ...
                BATCH_SIZE = 5  # parallel requests
                t0 = time.time()
                ...
                for batch_start in range(start_idx, len(data["cids_test"]), BATCH_SIZE):
                    ...
                    if not budget.check():
                        np.savez(checkpoint_file, preds=np.array(preds))
                        print(f"  Budget exceeded at {len(preds)}/{len(data['cids_test'])}", flush=True)
                        break

                auc = roc_auc_score(data["y_test"], preds)
                results[tag] = auc
                print(f"  {m['name']} {strategy}×{enrichment}: AUC={auc:.4f}", flush=True)
...
def run_rq3_cot_effect(dataset_name, api_key=None):
    """RQ3 CoT Effect: thinking on/off for DeepSeek-R1 and Qwen3.6."""
    print(f"\n{'='*60}")
    ...
    for mk in thinking_models:
        ...
        for thinking in [False, True]:
            ...
            if cache_file.exists():
                cached = json.load(open(cache_file))
                results[tag] = cached["auc"]
                print(f"  {m['name']} {mode}: AUC={cached['auc']:.4f} (cached)", flush=True)
                continue

            print(f"\n  --- {m['name']} {mode} + kNN CoT ---", flush=True)
            ...
            if checkpoint_file.exists():
                ...
                preds = list(ckpt["preds"])
                start_idx = len(preds)
                print(f"  Resuming from checkpoint: {start_idx}/{len(data['cids_test'])}", flush=True)
            else:
                preds = []
                start_idx = 0
            ...
            for i in range(start_idx, len(data["cids_test"])):
                ...
                if not budget.check():
                    np.savez(checkpoint_file, preds=np.array(preds))
                    print(f"  Budget exceeded. Checkpoint saved at {len(preds)}/{len(data['cids_test'])}", flush=True)
                    break

            auc = roc_auc_score(data["y_test"], preds)
            results[tag] = auc
            print(f"  {m['name']} {mode}: AUC={auc:.4f}", flush=True)
...
if __name__ == "__main__":
    ...
    print(f"Budget limit: ${args.budget:.2f}", flush=True)
